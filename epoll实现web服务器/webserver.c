...
int main()
{
	//注册信号处理函数，测试：10s内，将正在向cfd写数据的连接关闭，就会导致服务器退出，并发送sigpipe信号
	//signal(SIGPIPE,sighandler);
	
	//若web服务器给浏览器发送数据的时候, 浏览器已经关闭连接, 
	//则web服务器就会收到SIGPIPE信号,该信号的默认处理动作是终止进程，即关闭当前服务器进程
	
	//1、使用signal忽略SIGPIPE信号
	//signal(SIGPIPE,SIG_IGN);
	
	//2、使用sigaction函数忽略SIGPIPE信号
	...
	//3、使用信号集相关函数阻塞SIGPIPE信号
	//sigset_t set;//信号集
	//sigemptyset(&set);//将信号集所有位，置0
	//sigaddset(&set,SIGPIPE);//将SIGPIPE信号对应的位置1
	//sigprocmask(SIG_BLOCK,&set,NULL);//将信号集加入到阻塞信号集中
...
int http_request(int cfd, int epfd)
{
	...
	else //若文件存在
	{
		...
		else if(S_ISDIR(st.st_mode))
		{
			...
			sleep(10);//测试：10s内，将正在向cfd写数据的连接关闭，就会导致服务器退出，并发送sigpipe信号
			//解决方法1：忽略sigpipe信号
			//解法方法2：阻塞sigpipe信号
