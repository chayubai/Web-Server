 ...
 */
//计算机的底层也是二进制编码，所以需要把浏览器解析出来的中文字符串编码变为计算机能够识别的二进制编码(也就是整型值)
void strdecode(char *to, char *from)//from要转换的字符串，to将转换的字符串存入to指针指向的空间
{
    for ( ; *from != '\0'; ++to, ++from) {
			//isxdigit判断是否为十六进制
    ...
    *to = '\0';//to指向了from的空间，将from内容进行覆盖，但是一定需要一个'\0'，否则后面还有from未覆盖的内容
	//将原来from每两个字节的两个字符，转为一个字节存放
}
